...
대부분 블로그 포스팅에서는 @Provides를 많이 사용합니다. 하지만 @Provides를 사용하려면 항상 Module을 따로 정의해야하는 불편함이 있습니다. 그럼 언제 어떤 방법으로 주입하고 싶은 객체를 정의하면 좋을지 한번 살펴보겠습니다.
...
생성자코드 앞에 @Inject 어노테이션을 사용하여 객체 주입이 가능한 경우 따로 @Provides를 사용할 필요가 없습니다. Constructor injection 은 주입하고자 하는 객체 클래스의 생성자가 내 프로젝트내에 있어서 접근/수정이 가능하면 사용이 가능합니다. 반대로 말하면 Calendar.getInstance(), Builder 패턴을 이용하는 클래스 또는 외부 라이브러리에서 제공되는 클래스라 <u>개발자가 생성자를 수정하지 못할때</u> Constructor Injection을 사용할 수 없다는 의미가 되기도 합니다.
